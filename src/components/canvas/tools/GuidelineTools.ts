...
import { IStaticObject, IStaticWorkarea } from '../Canvas';
...
export interface IGuidelineTools extends ITools {
    ctx?: CanvasRenderingContext2D;
    viewportTransform?: number[];
    zoom?: number;
    verticalLines?: ICoords[];
    horizontalLines?: ICoords[];
    aligningLineOffset?: number;
    aligningLineMargin?: number;
    aligningLineWidth?: number;
    aligningLineColor?: Color;
    init?(): void;
    drawVerticalLine?(line: any): void;
    drawHorizontalLine?(line: any): void;
    drawLine?(x1: number, y1: number, x2: number, y2: number): void;
    isInRange?(v1: number, v2: number): boolean;
    movingGuidelines?(target: IStaticObject): void;
}

class GuidelineTools extends Tools implements IGuidelineTools {
    ctx: CanvasRenderingContext2D;
    viewportTransform: number[];
    zoom: number;
    verticalLines: ICoords[];
    ...
    drawVerticalLine = (coords: ICoords) => {
        this.drawLine(
            coords.x + 0.5,
    ...
    drawHorizontalLine = (coords: ICoords) => {
        this.drawLine(
    ...
    movingGuidelines = (target: IStaticObject) => {
        const canvasObjects = this.canvas.getObjects() as IStaticObject[];
        ...
        for (let i = canvasObjects.length; i--;) {
            ...
            if (this.isInRange(objectLeft - (objectWidth / 2), activeObjectLeft - (activeObjectWidth / 2))) {
                verticalInTheRange = true;
                if (canvasObjects[i].id === 'workarea') {
                    const workarea = canvasObjects[i] as IStaticWorkarea;
                    const y1 = -5000;
                    const y2 = 5000;
    ...
    scalingGuidelines = (target: IStaticObject) => {
        // TODO...
    }
}

export default GuidelineTools;
